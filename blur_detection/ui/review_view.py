...
LANG_REVIEW = {
    "EN": {
        ...
        "p_folder": "Path to Filtered Images Folder",
...
def render_review_tab():
    ...
    st.markdown("""
        <style>
        /* 1. Force structural column blocks to allow overflow escaping */
        ...
        /* 2. Setup standard inline image block properties */
        ...
        /* FIX: Radical Viewport Isolation for 80% Absolute Screen Centering */
        div[data-testid="stImage"] img:hover { 
            position: fixed !important; 
            top: 50vh !important;       /* Coordinates fixed relative to window view frame */
            left: 50vw !important;
            transform: translate(-50%, -50%) !important; /* Pulls exact center of image onto center coordinate */
            
            width: 80vw !important;      /* Forces scale dimension properties up to 80% */
            ...
            z-index: 999999 !important; /* Floats above all native elements and containers */
    ...
    with col_csv:
        csv_path = st.text_input(ln["p_csv"], value=str(Path.home() / "566-qa-2/filtered_images/600D/predictions_filtered.csv"))
    with col_img:
        img_folder = st.text_input(ln["p_folder"], value=str(Path.home() / "566-qa-2/filtered_images/600D"))
    with col_ref:
    ...
    known_system_statuses = [
        "Pass (Program Pass)",
        "Pass (Human Reviewed)",
        "Review (Flagged for Double Check)",
        "FLC Required (Human Reviewed)",
        "FLC Required (Error/Warning)"
    ]
    all_statuses = sorted(list(set(st.session_state.live_statuses.values()).union(set(known_system_statuses))))
    
    selected_statuses = st.multiselect(
        ln["filter_lbl"],
        options=all_statuses,
        default=[s for s in all_statuses if "Pass" not in s],
        help=ln["filter_help"]
    )
    ...
    if current_sn not in st.session_state.annotations:
        if json_path.exists():
            try:
                ...
                initial_flc = "Yes" if (has_pipeline_error or saved_status == "FLC Required (Human Reviewed)") else "No"
                st.session_state.annotations[current_sn] = {
                    ...
                    'touched': False if saved_status in ["Review (Flagged for Double Check)", "FLC Required (Error/Warning)", "Pass (Program Pass)"] else True
                }
            except Exception:
        ...
        if current_sn not in st.session_state.annotations:
            ...
            st.session_state.annotations[current_sn] = {
                'pos 1': get_default_annotation(1), 'pos 3': get_default_annotation(3),
                'pos 5': get_default_annotation(5), 'pos 7': get_default_annotation(7), 'pos 9': get_default_annotation(9),
    ...
    def auto_save_and_compile_master():
        ...
        if has_pipeline_error: 
            ...
            live_status = st.session_state.live_statuses.get(current_sn, current_row.get("Status", "Review (Flagged for Double Check)"))
        else: 
        ...
        updated_json_payload = {"SN": current_sn, "Status": live_status, "Error Description": error_desc, "positions": {}}
        for p in positions:
        ...
        for index, row in df_master.iterrows():
            ...
            if loop_json_path.exists():
                try:
                    ...
                    action_status = "FLC Required (Error/Warning)" if loop_has_error else d.get("Status", "Review (Flagged for Double Check)")
                except Exception:
                ...
                if sn_loop in st.session_state.annotations:
                    ...
                    anno = {f'pos {p}': cached[f'pos {p}'] for p in positions}
                    if loop_has_error: action_status = "FLC Required (Error/Warning)"
                    elif not cached['touched']: action_status = st.session_state.live_statuses.get(sn_loop, row.get("Status", "Review (Flagged for Double Check)"))
                    else: action_status = "FLC Required (Human Reviewed)" if cached['need_flc'] == "Yes" else "Pass (Human Reviewed)"
                else:
    ...
    def render_control_deck(location_key):
        st.write(f"### {ln['record']}: {st.session_state.review_idx + 1} / {total_sns}")
        ...
        if has_pipeline_error: 
            ...
            live_action_status = st.session_state.live_statuses.get(current_sn, current_row.get("Status", "Review (Flagged for Double Check)"))
        else: 
        ...
        with f_col1:
            if st.button(ln["flc_y"], key=f"flc_y_{location_key}_{current_sn}", type="primary" if (is_touched and current_flc == "Yes") else "secondary", width='stretch'):
                st.session_state.annotations[current_sn]['need_flc'] = "Yes"; st.session_state.annotations[current_sn]['touched'] = True
                auto_save_and_compile_master(); st.rerun()
        with f_col2:
            if st.button(ln["flc_n"], key=f"flc_n_{location_key}_{current_sn}", type="primary" if (is_touched and current_flc == "No" and not has_pipeline_error) else "secondary", width='stretch', disabled=has_pipeline_error):
                st.session_state.annotations[current_sn]['need_flc'] = "No"; st.session_state.annotations[current_sn]['touched'] = True
                auto_save_and_compile_master(); st.rerun()
        ...
        with nav_col1:
            if st.button(ln["prev"], key=f"nav_prev_{location_key}", width='stretch') and st.session_state.review_idx > 0:
                auto_save_and_compile_master(); st.session_state.review_idx -= 1; st.rerun()
        with nav_col2:
            if st.button(ln["next"], key=f"nav_next_{location_key}", width='stretch') and st.session_state.review_idx < total_sns - 1:
                auto_save_and_compile_master(); st.session_state.review_idx += 1; st.rerun()
                    ...
                    with sub_c1:
                        ...
                        for cls_label in classes:
                            if st.button(cls_label.upper(), key=f"lbl_{current_sn}_{pos}_{cls_label}", type="primary" if (current_selection == cls_label) else "secondary", width='stretch'):
                                ...
                                st.session_state.annotations[current_sn]['need_flc'] = "Yes" if (has_pipeline_error or active_labels.count('n') >= 1 or active_labels.count('sn') >= 3) else "No"
                                auto_save_and_compile_master(); st.rerun()
                        st.markdown('</div>', unsafe_allow_html=True)
                        
                    with sub_c2:
                        # Left-aligned Meta Header Data
