...
# 1. Check if the setup script was run by verifying the assets folder existence
...
# 2. Check for missing neural network weights inside the assets directory
REQUIRED_WEIGHTS=(
...
# 4. Initialize shell workspace for Conda commands safely
# This handles initializing conda inside non-interactive bash subshells
eval "$(conda shell.bash hook)"

# 5. Activate environment and boot up the server core
